...
def _save_tracker_output_oxuva(seq: Sequence, tracker: Tracker, output: dict, inf_num=None):
    if inf_num is None:
        ...
        if not os.path.exists(os.path.join(tracker.results_dir, str(inf_num))):
            os.makedirs(os.path.join(tracker.results_dir, str(inf_num)))
        ...
        pred_file = os.path.join(tracker.results_dir, str(inf_num), '{}_{}.csv'.format(vid_id, obj_id))
...
def _save_tracker_output(seq: Sequence, tracker: Tracker, output: dict, inf_num = None):
    ...
    if inf_num is None:
        ...
        base_results_path = os.path.join(tracker.results_dir, str(inf_num), seq.name)
        segmentation_path = os.path.join(tracker.segmentation_dir, str(inf_num), seq.name)
...
def run_sequence(seq: Sequence, tracker: Tracker, debug=False, visdom_info=None, inf_num=None):
    ...
    if inf_num is None:
        ...
        def _results_exist():
            if seq.dataset == 'oxuva':
                ...
                bbox_file = '{}/{}/{}.txt'.format(tracker.results_dir, str(inf_num), seq.name)
                return os.path.isfile(bbox_file)
            else:
                bbox_files = ['{}/{}/{}_{}.txt'.format(tracker.results_dir, str(inf_num), seq.name, obj_id) for obj_id in seq.object_ids]
                missing = [not os.path.isfile(f) for f in bbox_files]
                return sum(missing) == 0
        ...
        print('Tracker: {} {} {} ,  Inf number: {} Sequence: {}'.format(tracker.name, tracker.parameter_name, tracker.run_id, str(inf_num), seq.name))
        ...
        if not debug:
            if seq.dataset == 'oxuva':
                _save_tracker_output_oxuva(seq, tracker, output, inf_num= inf_num)
            else:
                _save_tracker_output(seq, tracker, output, inf_num= inf_num)


def run_dataset(dataset, trackers, debug=False, threads=0, visdom_info=None, multiple_inf_num=None):
    ...
    if multiple_inf_num is None:
        print('Evaluating {:4d} trackers on {:5d} sequences'.format(len(trackers), len(dataset)))
    else:
        print('Evaluating {:4d} trackers on {:5d} sequences for {:2d} times'.format(len(trackers), len(dataset), multiple_inf_num))
    ...
    if mode == 'sequential':
        if multiple_inf_num is None:
            ...
            for inf_num in range(multiple_inf_num):
                for seq in dataset:
                    for tracker_info in trackers:
                        run_sequence(seq, tracker_info, debug=debug, visdom_info=visdom_info, inf_num = inf_num)
    elif mode == 'parallel':
